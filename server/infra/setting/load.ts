...
export function camelCaseMap(config: { [name: string]: TOMLAvailableValue }): {
        ...
        anyFileConfSourceExisted
            ? defaultsDeep(
                  ...(fileConfSources.map((src) => {
                      if (src.existed) {
                          const conf = loadConfigFile(NODE_CONFIG_DIR, src.fullName);
                          if (
                              conf?.nacos?.config_client?.i18n_translation_group &&
                              I18N_NACOS_GROUP
                          ) {
                              conf.nacos.config_client.i18n_translation_group = I18N_NACOS_GROUP;
                          }
                          return conf;
                      }
                      return null;
