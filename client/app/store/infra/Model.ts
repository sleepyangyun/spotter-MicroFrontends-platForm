 ...
 * @method getModel
 */
export function LoadWatcherModel<T extends { [action: string]: boolean }>(loadMap: T) {
    /**
...
interface WithAutoLoadWatchersProps<T extends Record<string, any>, O> {
    name: string;
    properties: T;
    overridesInitWatcher?: O;
}

export default function Model<T extends Record<string, any>, O extends Record<string, any>>({
    name,
    properties,
    overridesInitWatcher,
}: WithAutoLoadWatchersProps<T, O>) {
    return types
        .model(name, {
            ...properties,
            loadWatcher: LoadWatcherModel({
        ...
        .views((self) => ({
            loadWatchers(keys: (keyof T | keyof O)[]) {
                return keys.some((key) => self.loadWatcher.get(key as string));
            },
        }));
}
